...
ユーザーから提供された修正指示に基づいて、backend / frontend の定義構造をインテリジェントに編集・出力してください。自然言語での対話と説明は簡潔にまとめ、\`\`\`json コードブロックを必ず含めてください。
`;
...
function App() {
  ...
  const handleSendMessage = async (userMessage: string) => {
    ...
    setProjectState(prev => ({
      ...prev,
      chatHistory: [...updatedHistory, { role: 'assistant', content: '' }]
    }));

    try {
      // システムプロンプトを先頭にインジェクトする (ただし毎回インジェクトするとコンテキスト肥大化するため、BFFのやりとりで1回設定)
      const systemMsg: ChatMessage = { role: 'user', content: SYSTEM_PROMPT };
      
      // ユーザーに見せないシステム指示と、現在までのメタデータをコンテキストに追加
...
`;
      ...
      const response = await fetch('/api/chat', {
        ...
        body: JSON.stringify({
          ...
          model: 'google/gemini-flash-1.5' // デフォルト
        })
      });
      ...
      while (true) {
        ...
        // chunkには `data: {...}` または SSE 特有の複数行テキストが入っている
        ...
        for (const line of lines) {
          ...
          if (trimmed.startsWith('data: ')) {
            ...
            try {
              ...
              // 不完全なJSON行の場合は一旦スルー (次の chunk で連結されるため)
            }
          }
        }
      }

      // ストリーム完了後、メッセージ全体から JSON コードブロックを抽出してメタデータを更新
      parseAndSetMetadata(aiContentAccumulator);
      ...
      setProjectState(prev => {
        ...
        const lastMsg = nextHistory[nextHistory.length - 1];
        if (lastMsg && lastMsg.role === 'assistant') {
          lastMsg.content = `【エラー】接続に失敗しました。\n理由: ${error.message || '不明なエラー'}\n\nサーバー側で .env ファイルに OPENROUTER_API_KEY が正しく設定されていること、またローカルBFFサーバーが起動しているか確認してください。`;
        }
        return { ...prev, chatHistory: nextHistory };
      });
    } finally {
  ...
  // 生成されたテキストからJSONブロックを取り出し、メタデータを設定する
  const parseAndSetMetadata = (fullText: string) => {
    try {
      ...
      const backend: BackendMetadata = parsed.backend;
      const frontend: FrontendMetadata = parsed.frontend;

      if (backend && frontend) {
        setProjectState(prev => ({
          ...prev,
          backendMetadata: backend,
          frontendMetadata: frontend
        }));
        console.log('Successfully synced metadata from AI response.');
      }
    } catch (e) {
      console.warn('Failed to parse generated JSON from stream content', e);
    }
  };
